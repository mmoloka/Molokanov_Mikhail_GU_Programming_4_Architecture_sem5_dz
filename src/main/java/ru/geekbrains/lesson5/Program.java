...
public class Program {
    ...
    public static void main(String[] args) {
        ...
        while (f){
            ...
            System.out.println("8. Добавить в проект новую текстуру");
            System.out.println("9. Удалить из проекта текстуру");
                ...
                try {
                    switch (no) {
                        ...
                        case 7:
                            ...
                            break;
                        case 8:
                            editor3D.addTexture(newTexture);
                            break;
                        case 9:
                            editor3D.checkProjectFile();
                            System.out.println();
                            ...
                            if (scanner.hasNextInt()) {
                                ...
                                scanner.nextLine();
                                editor3D.removeTexture(textureId);
                            }
                            else {
                                System.out.println();
...
class Editor3D implements UILayer{
    ...
    public void addTexture(Texture newTexture) {
        ...
        databaseAccess.addTexture(newTexture);
        ArrayList<Texture> textures = (ArrayList<Texture>)businessLogicalLayer.getAllTextures();
        System.out.println();
        System.out.printf("Добавлена новая текстура #%d\n", textures.get(textures.size() - 1).getId());
    }
    ...
    public void removeTexture(int num) {
        ...
        try {
            databaseAccess.removeTexture(num);
            System.out.println();
            System.out.printf("Текстура #%d успешно удалена\n", num);
        } catch (Exception e) {
            System.out.println();
            System.out.println("Текстуры с таким номером не существует в проекте");
        }
...
class EditorDatabaseAccess implements DatabaseAccess{
    ...
    @Override
    public void addTexture(Texture newTexture) {
        editorDatabase.getAll().add(newTexture);
    }

    @Override
    public void removeTexture(int num) throws Exception {
        List<Texture> textures = (ArrayList<Texture>)getAllTextures();
        if(textures.contains(textures.get(num))) editorDatabase.getAll().remove(textures.get(num)) ;
...
interface DatabaseAccess{
    void addTexture(Texture newTexture);
    void removeTexture(int num) throws Exception;
